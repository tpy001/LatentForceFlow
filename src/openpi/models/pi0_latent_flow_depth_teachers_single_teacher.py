...
class Pi0LatentFlowDepthTeachers(_model.BaseModel):
    ...
    def _compress_joint_visuals(self, obs: _model.Observation):
        return jnp.concatenate([self._compress_visuals(obs, "flow"), self._compress_visuals(obs, "depth")], axis=1)
    ...
    def embed_joint_teacher_suffix(self, obs, noisy_actions, timestep):
        state = self.state_proj_flow_teacher(obs.state)[:, None, :]
        visual = self._compress_joint_visuals(obs)
        actions, adarms = self._embed_action_tokens(noisy_actions, timestep, "flow_teacher")
        tokens = jnp.concatenate([state, visual, actions], axis=1)
        token_count = self.flow_token_count + self.depth_token_count
        ar_mask = jnp.array([False] + ([False] * token_count) + [True] + ([False] * (actions.shape[1] - 1)))
        return tokens, jnp.ones(tokens.shape[:2], dtype=jnp.bool_), ar_mask, adarms

    def embed_empty_teacher_suffix(self, obs, noisy_actions, timestep):
        state = self.state_proj_depth_teacher(obs.state)[:, None, :]
        actions, adarms = self._embed_action_tokens(noisy_actions, timestep, "depth_teacher")
        tokens = jnp.concatenate([state, actions], axis=1)
        ar_mask = jnp.array([False] + [True] + ([False] * (actions.shape[1] - 1)))
        return tokens, jnp.ones(tokens.shape[:2], dtype=jnp.bool_), ar_mask, adarms
    ...
    def _forward_all_streams(
        ...
        full_attn = jnp.concatenate([prefix_row, student_row, flow_row], axis=1)
        ...
        positions = jnp.concatenate([prefix_positions, student_positions, flow_positions], axis=1)
        (outputs, layers), _ = self.PaliGemma.llm(
            [prefix_tokens, student_tokens, flow_tokens, None],
            mask=full_attn,
            positions=positions,
            adarms_cond=[None, student_adarms, flow_adarms, None],
    ...
    def compute_loss_with_stats(self, rng, observation, actions, *, train=False, train_progress=None):
        ...
        flow_tokens, flow_mask, flow_ar_mask, flow_adarms = self.embed_joint_teacher_suffix(
            observation, x_t_action, time
        )
        ...
        _, student_out, flow_out, _ = outputs
        ...
        depth_teacher_action_loss = jnp.zeros_like(student_action_loss)
        ...
        teacher_flow_slice = slice(1, 1 + self.flow_token_count)
        teacher_depth_slice = slice(1 + self.flow_token_count, 1 + self.flow_token_count + self.depth_token_count)
        ...
        for layer_ordinal, layer in enumerate(layers):
            _, student_hidden, flow_hidden, _ = layer
            student_flow_hidden = self._project_flow_distill(student_hidden[:, flow_slice, :], layer_ordinal)
            teacher_flow_hidden = jax.lax.stop_gradient(flow_hidden[:, teacher_flow_slice, :])
            flow_losses.append(
            ...
            teacher_depth_hidden = jax.lax.stop_gradient(flow_hidden[:, teacher_depth_slice, :])
            depth_losses.append(
                self._cosine_distance_masked(
