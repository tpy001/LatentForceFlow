...
class Pi0LatentFlowDepthTeachers(_model.BaseModel):
    ...
    def _forward_student_with_prefix(
        self,
        prefix_tokens,
        prefix_mask,
        prefix_ar_mask,
        ...
        prefix_attn = make_attn_mask(prefix_mask, prefix_ar_mask)
        student_attn = make_attn_mask(student_mask, student_ar_mask)
        student_prefix = einops.repeat(prefix_mask, "b p -> b s p", s=student_len)
        student_prefix = jnp.logical_and(student_prefix, student_mask[:, :, None])
        ...
        student_row = jnp.concatenate([student_prefix, student_attn], axis=-1)
        ...
        (outputs, layers), kv_cache = self.PaliGemma.llm(
            [prefix_tokens, student_tokens, None, None],
            mask=full_attn,
            positions=positions,
        ...
        student_layers = tuple(layer[1] for layer in layers)
        return student_out, student_layers, kv_cache

    def _forward_teachers_with_stopped_prefix_cache(
        self,
        prefix_mask,
        prefix_kv_cache,
        flow_tokens,
        ...
        prefix_kv_cache = jax.tree.map(
            lambda x: jax.lax.stop_gradient(x[:, :, :p_len, ...]),
            prefix_kv_cache,
        )
        ...
        full_attn = jnp.concatenate([flow_row, depth_row], axis=1)

        prefix_len = jnp.sum(prefix_mask, axis=-1)[:, None]
        flow_positions = prefix_len + jnp.cumsum(flow_mask, axis=-1) - 1
        depth_positions = prefix_len + jnp.cumsum(depth_mask, axis=-1) - 1
        positions = jnp.concatenate([flow_positions, depth_positions], axis=1)

        (outputs, layers), _ = self.PaliGemma.llm(
            [None, None, flow_tokens, depth_tokens],
            mask=full_attn,
            positions=positions,
            kv_cache=prefix_kv_cache,
            adarms_cond=[None, None, flow_adarms, depth_adarms],
            return_layer_indices=self.distill_layer_indices,
        )
        _, _, flow_out, depth_out = outputs
        flow_layers = tuple(layer[2] for layer in layers)
        depth_layers = tuple(layer[3] for layer in layers)
        return flow_out, depth_out, flow_layers, depth_layers
    ...
    def compute_loss_with_stats(self, rng, observation, actions, *, train=False, train_progress=None):
        ...
        student_out, student_layers, prefix_kv_cache = self._forward_student_with_prefix(
            prefix_tokens,
            prefix_mask,
            prefix_ar_mask,
            student_tokens,
        ...
        flow_out, depth_out, flow_layers, depth_layers = self._forward_teachers_with_stopped_prefix_cache(
            prefix_mask,
            prefix_kv_cache,
            flow_tokens,
            flow_mask,
        ...
        flow_losses = []
        depth_losses = []
        for layer_ordinal, (student_hidden, flow_hidden, depth_hidden) in enumerate(
            zip(student_layers, flow_layers, depth_layers, strict=True)
        ):
            flow_losses.append(
