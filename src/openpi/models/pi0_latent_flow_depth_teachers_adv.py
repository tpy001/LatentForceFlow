...
class Pi0LatentFlowDepthTeachers(_model.BaseModel):
    ...
    def __init__(self, config: pi0_config.Pi0LatentFlowDepthTeachersConfig, rngs: nnx.Rngs):
        ...
        self._init_adv_discriminator("flow", self.flow_teacher_width, rngs)
        self._init_adv_discriminator("depth", self.depth_teacher_width, rngs)
    ...
    def _init_adv_discriminator(self, name: str, width: int, rngs: nnx.Rngs) -> None:
        setattr(self, f"{name}_adv_disc_bottleneck", nnx.Linear(width, 256, rngs=rngs))
        setattr(self, f"{name}_adv_disc_conv_0", nnx.Conv(256, 128, kernel_size=(2, 3), padding="SAME", rngs=rngs))
        setattr(self, f"{name}_adv_disc_conv_1", nnx.Conv(128, 64, kernel_size=(2, 3), padding="SAME", rngs=rngs))
        setattr(self, f"{name}_adv_disc_out", nnx.Linear(64, 1, rngs=rngs))
    ...
    @staticmethod
    def _gradient_reverse(x, scale: float = 1.0):
        scale = jnp.asarray(scale, dtype=x.dtype)
        return jax.lax.stop_gradient((1.0 + scale) * x) - scale * x
    ...
    def _adv_discriminator_logits(self, name: str, hidden):
        hidden = hidden.astype(jnp.float32)
        hidden = getattr(self, f"{name}_adv_disc_bottleneck")(hidden)
        hidden = nnx.swish(hidden)
        hidden = getattr(self, f"{name}_adv_disc_conv_0")(hidden)
        hidden = nnx.swish(hidden)
        hidden = getattr(self, f"{name}_adv_disc_conv_1")(hidden)
        hidden = nnx.swish(hidden)
        hidden = jnp.mean(hidden, axis=(1, 2))
        return jnp.squeeze(getattr(self, f"{name}_adv_disc_out")(hidden), axis=-1)

    def _global_adversarial_alignment_loss(self, name: str, student, teacher):
        student = student.astype(jnp.float32)
        teacher = teacher.astype(jnp.float32)

        student_disc_logits = self._adv_discriminator_logits(name, self._gradient_reverse(student))
        teacher_disc_logits = self._adv_discriminator_logits(name, jax.lax.stop_gradient(teacher))
        student_prob = jax.nn.sigmoid(student_disc_logits)
        teacher_prob = jax.nn.sigmoid(teacher_disc_logits)
        student_loss = jnp.square(student_prob)
        teacher_loss = jnp.square(teacher_prob - 1.0)
        return 0.5 * (student_loss + teacher_loss)
    ...
    def compute_loss_with_stats(self, rng, observation, actions, *, train=False, train_progress=None):
        ...
        flow_losses = []
        depth_losses = []
        flow_student_features = []
        flow_teacher_features = []
        depth_student_features = []
        depth_teacher_features = []
        for layer_ordinal, layer in enumerate(layers):
            ...
            flow_student_features.append(student_flow_hidden)
            flow_teacher_features.append(teacher_flow_hidden)
            flow_losses.append(
            ...
            depth_student_features.append(student_depth_hidden)
            depth_teacher_features.append(teacher_depth_hidden)
            depth_losses.append(
                self._cosine_distance_masked(
        ...
        future_flow_adv_loss = self._global_adversarial_alignment_loss(
            "flow",
            jnp.stack(flow_student_features, axis=1),
            jnp.stack(flow_teacher_features, axis=1),
        )
        future_depth_adv_loss = self._global_adversarial_alignment_loss(
            "depth",
            jnp.stack(depth_student_features, axis=1),
            jnp.stack(depth_teacher_features, axis=1),
        )
        total_loss = (
            ...
            + self.future_flow_contrast_loss_weight * future_flow_adv_loss
            + self.future_depth_contrast_loss_weight * future_depth_adv_loss
        )
        stats = {
            ...
            "loss/adv_future_flow": future_flow_adv_loss,
            "loss/adv_future_depth": future_depth_adv_loss,
