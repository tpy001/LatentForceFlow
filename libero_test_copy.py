...
class _FatalEvaluationError(RuntimeError):
    pass
    ...
    try:
        ...
        for task_id in tqdm.tqdm(range(num_tasks_in_suite)):
            ...
            try:
                    ...
                    while t < max_steps + args.num_steps_wait:
                        try:
                            ...
                            if not action_plan:
                                ...
                                # Query model to get action. Policy/server errors should abort the suite instead of
                                # being counted as rollout failures.
                                try:
                                    ...
                                    logging.exception("Policy inference failed; aborting evaluation.")
                                    raise _FatalEvaluationError("Policy inference failed.") from e
                ...
                _close_resource(env, f"LIBERO env for task {task_id}")

        logging.info(f"Total success rate: {float(total_successes) / float(total_episodes)}")
        logging.info(f"Total episodes: {total_episodes}")
    finally:
        _close_resource(client, "websocket client")
...
def _close_resource(resource, name: str) -> None:
    ...
    if close is None and hasattr(resource, "_ws"):
        close = getattr(resource._ws, "close", None)
    if close is None:
        logging.info(f"No close method found for {name}.")
        return
    try:
        close()
    except Exception as e:
        logging.warning(f"Failed to close {name}: {e}")
    else:
